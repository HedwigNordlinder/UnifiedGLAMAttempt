...
function simulate_2d(rng; npatients = 4, nobs = 50)
    μ0 = ([-1.5, 0.6], [1.0, -0.9])
    Σ = ([0.35 0.16; 0.16 0.55], [0.55 -0.14; -0.14 0.30])
    ...
    for i in 1:npatients
        shift = 0.35 .* randn(rng, 2)
        M = hcat(μ0[1] .+ shift .+ 0.15 .* randn(rng, 2), μ0[2] .+ shift .+ 0.15 .* randn(rng, 2))
        Xi = Matrix{Float64}(undef, 2, nobs)
...
function plot_sample(data, state, truth; path = "latent_2d_sample.png")
    ...
    for i in 1:npatients
        Xi, zi = data.X[i], state.z[i]
        title = @sprintf("Patient %d, pi=%.2f", i, state.model.pi[i])
        plot!(plt[i]; xlabel = "x1", ylabel = "x2", aspect_ratio = :equal, title)
        for k in 1:2
            keep = k == 1 ? zi : .!zi
        ...
        for k in 1:2
            Σhat = state.model.Sigma[k] / state.model.lambda[i, k]
            μhat = state.model.mu[i][:, k]
            for lev in levels
            ...
            Etrue = ellipse_points(truth.model.mu[i][:, k], truth.model.Sigma[k] / truth.model.lambda[i, k]; level = 0.8)
...
function main()
    ...
    data, truth = simulate_2d(rng)
    prior = default_prior(data)
    out = gibbs(rng, data, prior; nsweeps = 1_000)
    path = plot_sample(data, out.state, truth)
    @printf("saved %s\n", abspath(path))
